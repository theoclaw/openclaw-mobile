...
public final class TermuxInstaller {
    ...
    public static void setupBootstrapIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                PowerManager.WakeLock wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "ai.clawphones.agent:bootstrap");
                ...
                try {
                    ...
                    // Create ClawPhones install script and environment
                    createClawPhonesScripts();
    ...
    /**
     * Creates the ClawPhones installation script and environment setup.
     *
     * Creates:
     * 1. $PREFIX/share/clawphones/install.sh — standalone installer with structured output
    ...
    private static void createClawPhonesScripts() {
        try {
            ...
            File clawphonesDir = new File(TERMUX_PREFIX_DIR_PATH + "/share/clawphones");
            if (!clawphonesDir.exists()) {
                clawphonesDir.mkdirs();
            }

            File installScript = new File(clawphonesDir, "install.sh");
            String installContent =
                "#!" + com.termux.shared.termux.TermuxConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/bash\n" +
                "# ClawPhones install script — single source of truth\n" +
                "# Called by: GUI (ProcessBuilder) and terminal (profile.d)\n" +
                "# Outputs structured lines for GUI progress parsing.\n\n" +
                "LOGFILE=\"$HOME/clawphones-install.log\"\n" +
            ...
            File envScript = new File(profileDir, "clawphones-env.sh");
            String envContent =
                "# ClawPhones environment setup\n" +
                ...
                "if [ ! -f \"$HOME/.clawphones_installed\" ]; then\n" +
                "    echo \"\\U0001F4A7 Setting up ClawPhones...\"\n" +
                "    bash $PREFIX/share/clawphones/install.sh\n" +
                "fi\n";
            ...
            Logger.logInfo(LOG_TAG, "Created ClawPhones scripts in " + clawphonesDir.getAbsolutePath());

        } catch (Exception e) {
            Logger.logStackTraceWithMessage(LOG_TAG, "Failed to create ClawPhones scripts", e);
        }
    }
